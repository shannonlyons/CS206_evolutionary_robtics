    ...
    def Update_Hidden_Or_Motor_Neuron(self, neurons, synapses):
        ...
        for key in synapses:
            if key[1] == self.Get_Name():
                self.currSynapseWeight = synapses[key].Get_Weight()
                self.presynapticNeuronVal = neurons[key[0]].Get_Value()
                self.Allow_Presynaptic_Neuron_To_Influence_Me(self.currSynapseWeight, self.presynapticNeuronVal)

            self.Threshold()

    def Allow_Presynaptic_Neuron_To_Influence_Me(self, weight, value):

        self.multipliedWeightAndVal = weight * value
        self.Add_To_Value(self.multipliedWeightAndVal)
